...
startGame = {
  ...
  if (isServer) then {
      ...
      while {!gameFinished} do {
        ...
        if (!isNull _screenToUpdate) then {
          if ((count shuffledObjectiveTable) > currentObjective) then {
            ...
            sleep 5;
            _warningMessage = "WARP DRIVE ONLINE<br/><t color='#DA7422'>ENERGY SURGE DETECTED</t>";
            [_warningMessage] remoteExec ["veryImportantGOODStoryText", 0, false];
    ...
    //WARP DISRUPTION EXTRA PUNISHMENTS.
    [] spawn {
      while {!gameFinished} do {
        sleep (400 + floor(random 100));
        if (!gameFinished) then {
          _warningMessage = "<br/><t color='#DA7422'>WARP DISRUPTION DETECTED</t>";
          [_warningMessage] remoteExec ["veryImportantStoryText", 0, false];
          ["warpNoise"] remoteExec ["playMusic", 0, false];
          sleep 3;
          [] spawn scramblePunishment;
        };
      };
    };
    ...
    "lastPushedObjective" addPublicVariableEventHandler {
       if ((activeObjectives find lastPushedObjective) != -1) then {
         ...
         _whatsLeft = (currentObjective / 35) * 100;
         [("Warp is approx. " + str _whatsLeft + "% charged")] remoteExec ["shorterHint", 0, false];
         {
...
scramblePunishment = {
  [] spawn (selectRandom scramblePunishmentsList);
};
