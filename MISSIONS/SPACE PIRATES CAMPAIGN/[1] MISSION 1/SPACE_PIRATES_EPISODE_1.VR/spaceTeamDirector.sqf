...
//Hard verbs can be 0 (easy) or 4 (hard)
hardVerbs = 4;
...
startGame = {
  ...
  if (isServer) then {
    ...
    _warningMessage = "WARP DRIVE ENGAGED";
    [_warningMessage] remoteExec ["veryImportantGOODStoryText", 0, false];

    [] spawn {
      playSound3D ["a3\sounds_f\sfx\siren.wss", theActualShip];
      sleep 1;
      playSound3D ["a3\sounds_f\sfx\siren.wss", theActualShip];
      sleep 1;
      playSound3D ["a3\sounds_f\sfx\siren.wss", theActualShip];
      ...
      while {!gameFinished} do {
        ...
        if (!isNull _screenToUpdate) then {
          if ((count shuffledObjectiveTable) > currentObjective) then {
            ...
            [_warningMessage] remoteExec ["veryImportantGOODStoryText", 0, false];
            playSound3D [getMissionPath "sfx\warp.ogg", theActualShip];
            gameFinished = true;
            sleep 2;
