...
if (isServer) then {
  profileNamespace setVariable ["PIRATEVAR_scanInProgress", false];
	missionNamespace setVariable ["PIRATEVAR_lastSummonTime", -1800];
...
if (isServer) then {
  "startJackShackSummon" addPublicVariableEventHandler {
		...
		if (((missionNamespace getVariable "PIRATEVAR_lastSummonTime") + 1800) < time) then {
  		[summoningPlayer] spawn actuallySummonJackShack;
			missionNamespace setVariable ["PIRATEVAR_lastSummonTime",time];
		} else {
			_timeLeft = floor((1800 - (time - (missionNamespace getVariable "PIRATEVAR_lastSummonTime"))) / 60);
...
actuallySummonJackShack = {
	...
	[_summonedJackShack] call makePirateArsenal;
};
