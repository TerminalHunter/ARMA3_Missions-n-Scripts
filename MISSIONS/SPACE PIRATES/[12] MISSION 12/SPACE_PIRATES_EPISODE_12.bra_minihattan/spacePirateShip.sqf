...
},[],21,true,true,"","true",10,false,"",""];

vehicleSpawner addAction ["[BOX] Cargo Box", {
  ["Land_MetalCase_01_large_F"] call replicateNewVehicleFromPonyExpress;
...
vehicleSpawner addAction ["*ON BACKORDER* [MECH] Replicate Defender Mech", {
  //["BTLSTD_Defender_BLU_HC_UNSPACY"] call replicateNewVehicleFromPonyExpress;
  playSound3D [getMissionPath "sfx\buzz.ogg", vehicleSpawner];
...
replicateNewVehicleFromPonyExpress = {
  ...
  _newVic = _typeOfVehicle createVehicle [542.293,1132.58,0]; //Minihattan map is surrounded by water. This is set for the bottom left of the map, but on that background building island.
  _newVic setDir 90;
  _newVic setPosASL (respawnerLocation vectorAdd [0,0,0.50]);
  _newVic addAction ["Y.E.E.T.", {
    (_this select 0) setVelocity [0,0,10];
    sleep 0.4;
    (_this select 0) setVelocity [50,0,0]; //ship is now facing west
...
makeNudgeable = {
  ...
  [_thingToNudge, false] call ace_dragging_fnc_setDraggable;
  [_thingToNudge, false] call ace_dragging_fnc_setCarryable;
