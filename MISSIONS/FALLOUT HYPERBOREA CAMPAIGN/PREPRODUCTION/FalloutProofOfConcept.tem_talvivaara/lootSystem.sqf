...
mapApocalypsizing = {
	...
	//TODO are we really calling a hard disc read every second for these? That can be better.
	[]spawn{
	...
	//place just about every world object that isn't a ruin into the worldObject array
	//for later deletion/hiding and replacement
	[]spawn{
		...
		} forEach nearestTerrainObjects [partyInventory,["BUILDING","HOUSE","FENCE","WALL", "VIEW-TOWER","HIDE","FOUNTAIN"],worldSize*2,true];

	//OFFSET VALUES - after X number of objects are hidden, take a quick break to allow for other processes to occur
	//25 seems to be good for approx 20 fps and a load time of 3-4 minutes. [IN TESTING - actual server performs better]
	//higher values tank FPS but result in faster load times
		_origOffset = 25;
		_currOffset = _origOffset;
	// for every worldObject - hide the object
	// VERY EXPENSIVE, accounts for most of the load time
		{
			if (_currOffset > 0) then {
				_currOffset = _currOffset - 1;
				hideObjectGlobal _x;
			} else{
				sleep 0.02; //The biki says this is the smallest amount of time possible to sleep
				hideObjectGlobal _x;
				_currOffset = _origOffset;
			};
		} forEach _worldObjects;
		// pass the array of now hidden world objects to the functions that makes new ruins and fill them with loot
		[_worldObjects] call spreadFixedLoot;
		// set up caches of loot randomly spread out on the map
		[] call spreadLootCaches;
		//map mapApocalypsizing is now done - set map init to finished and let the players know
...
spreadFixedLoot = {
	...
	_num = 0;
	{
		if (!((getPos _x) isFlatEmpty [1, -1, 0.03, 8, -1] isEqualTo[])) then{ //THIS WORKS: if (!((getPos _x) isFlatEmpty [1, -1, 0.01, 9, -1] isEqualTo[])) then{
			...
			if (_objectNumber < 0) then {
				...
				_num = _num + 1;
				//_objectNumber = random 2; // this line means only approx 1/3 of possible positions have ruins in them
...
createLootBox = {
	...
	//code to delete loot box when empty

	/* OLD CODE ONLY WORKS ON SERVER
	_cacheLootBoxBox addEventHandler ["ContainerClosed",{
