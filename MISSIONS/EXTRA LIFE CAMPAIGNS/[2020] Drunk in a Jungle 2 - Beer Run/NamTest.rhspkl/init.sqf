...
//how to set the larger billboards
bigBill setObjectTextureGlobal [0, "eastereggbeerlabels\signs\toxicmeme.paa"];
bigBill setObjectTextureGlobal [1, "eastereggbeerlabels\signs\ICE.paa"];
bigBiller setObjectTextureGlobal [0, "eastereggbeerlabels\signs\ICE.paa"];
bigBiller setObjectTextureGlobal [1, "eastereggbeerlabels\signs\toxicmeme.paa"];

//WHEN NEW BEER LABELS ARE RELEASE, THIS ARRAY MUST BE UPDATED!
boozen = ["noBrand_Beer","normal_beer","fuch_beer","blastedBat_Sauce","hell_beer","love_beer"];
boozenNumber = 0;
...
beerBoxAction = ["Crack Open A Cold One", {
		...
		//check if player is attempting to drink and drive (or ride)
		_isInVic = vehicle _caller != _caller;
		//spawn a beer can into the world
		_coldOne = (selectRandom boozen) createVehicle (getPos _caller);
		//precalc the on-foot offset vector for the can
		...
		//this should probably use modulo instead, but this works for now
