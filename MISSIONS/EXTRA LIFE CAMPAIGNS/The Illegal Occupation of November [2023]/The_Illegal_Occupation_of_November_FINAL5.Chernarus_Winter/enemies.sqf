spawnEnemyGroup = {
	...
	if (_kindToSpawn == 1) then {
		for "_i" from 1 to _number do {
			...
			//eh, legos can't walk apparently
...

	{
		...
		switch (_kindToSpawn) do {
			...
			case 1: { //LEGO man -- but actually cheap santas?
				_x addUniform (selectRandom elfUniforms);
...
spawnEnemyGroupLEGO = {
	params["_number", "_position"];
	_newGroup = [_position, east, ["obh_man_adventurer","obh_man_adventurer","obh_man_adventurer","obh_man_adventurer","obh_man_adventurer","obh_man_adventurer"]] call BIS_fnc_spawnGroup;
	_newMarker = createMarker["LEGO"+str(random 10), _position];
	_newMarker setMarkerSize [50,50];
	{
		[_x] call unequipAll;
		...
		[_x] call equipXmas;
		_x setPos ([_newMarker] call grabRandomPosition);
	} forEach units _newGroup;
	_newGroup deleteGroupWhenEmpty true;
	_newGroup
};
...
xmasPrimaries = [
	...
	//["Xmas_Rifle_Xmas200","Xmas_Rifle_Xmas200_200Rnd"] wow this goes boom
];
