...
redeemReward = {
	...
	*/
	...
	//SCAN!
		if (_dataArray select 0 == "TwitchScan") then {
		//chat sideChat ((_dataArray select 1) + " has sent intel!");
		[chat, ((_dataArray select 1) + " has sent intel!")] remoteExec ["sideChat", 0, false];
		[_dataArray] spawn intelPackage;
	};
};
...
intelPackage = {
	params["_twitchData"];
	if (finaleStarted) then {
		//MECHA-RHIA SCARY HP QUERY
		[chat,"!!CHRISTMAS TITANOMECH DETECTED!!"] remoteExec ["sideChat", 0, false];
		[chat,"Titanomech Energy Signatures at " + str((mariahHP/666)*100) + "% strength"] remoteExec ["sideChat", 0, false];
		_mariahYPos = (getPos mariah) select 1;
		[chat,"Titanomech is " + str(floor(15000 - _mariahYPos)) + " meters from the October border."] remoteExec ["sideChat", 0, false];
	} else {
		//just the autodoc thingy
		[] call regularScan;
	};
};
...
regularScan = {
	...
	if (count _detectedEnemies > 0) then {
		...
		_warningMessage = _typeOfEnemy + " detected at approximate heading " + (str (floor _enemyHeading));
		[chat,_warningMessage] remoteExec ["sideChat",0,false];
	};
	_warningMessage2 = str(count _detectedEnemies) + " Christmas-time signatures detected within 500m vicinity.";
	[chat,_warningMessage2] remoteExec ["sideChat",0,false];
};
