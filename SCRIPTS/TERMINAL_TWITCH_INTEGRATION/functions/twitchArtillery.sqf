...
rawArtyStrike = {
	params ["_gridCoord"];
	[_gridCoord] remoteExec ["grabProperGridCoord", selectRandom allPlayers]; //THIS LINE PROBABLY BORK BUT WE BALL.
    //some poor sap gets to calculate the grid coord since the server can't. 
...
//I guess assume a zoos
    //unassume this immediately.
        //unassumed. now just grabs a random player. god help us if this doesn't work.
...
spawnExplosion = {
	params ["_position"];
	[_position, "Sh_155mm_AMOS", 125, 10, [3,8]] call BIS_fnc_fireSupportVirtual; //this can be super customized, though
};
